...
const AssessmentManager = {
   ...
   * 
   * LABEL MAPPING LOGIC:
   * - Extracts the user's selected numeric value from the range [1-5]
   * - Maps it to the corresponding option label from questions.js
   * - Formula: selectedOption = options[value - 1] (0-indexed array access)
   * 
   * UI DISPLAY:
   * - Min/Max labels are extracted from first and last options dynamically
   * - Selected label replaces numeric display for user-friendly UX
   * - Examples: "Selected: 6–7 hours" instead of "Selected: 3"
   */
  renderQuestions: () => {
  ...
  renderNavigation: () => {
    ...
    // Submit is handled by the form submit event
  },

  /**
   * Handle response change on slider
   * 
   * REAL-TIME LABEL UPDATE:
   * 1. Capture slider's numeric value (1-5 scale)
   * 2. Find matching option from QUESTIONS data structure
   * 3. Extract human-readable label (e.g., "6–7 hours")
   * 4. Update display immediately for user feedback
   * 
   * DATA PRESERVATION:
   * - Numeric value (1-5) is stored internally for scoring
   * - Label is displayed to user for clarity
   * - Both stored in responses array for later processing
   * 
   * EXAMPLE:
   * Slider value: 3
   * Matched option: { label: "6–7 hours", value: 3 }
   * Display text: "Selected: 6–7 hours"
   */
  onResponseChange: (e) => {
  ...
  validateStep: () => {
    const questions = QUESTIONS.filter(q => q.step === AssessmentManager.currentStep);
    console.log(`🔍 Validating step ${AssessmentManager.currentStep}, questions:`, questions.length);
  ...
  submitAssessment: async (event) => {
    ...
    console.log('📊 Current responses:', AssessmentManager.responses);
    
    if (!AssessmentManager.validateStep()) {
    ...
    AppManager.showLoader('Analyzing Your Responses', 'Sending to NeuroWell AI...');
    ...
    try {
      ...
      if (isBackendAvailable) {
        ...
        result = await APIService.submitAssessment(AssessmentManager.responses);
        console.log('✅ Backend submission successful:', result);

        // Store additional data locally for offline access
        StorageManager.saveAssessmentResponses(AssessmentManager.responses);
        StorageManager.saveWellnessScore(result);
        StorageManager.saveLastAssessmentDate();
        StorageManager.saveHistoricalData(result);
        localStorage.setItem('result', JSON.stringify(result));

        console.log('🎉 Assessment submitted to backend', result);
      } else {
        // Fallback to local calculation
        console.log('🔄 Backend unavailable, using local calculation');
        result = APIService.fallbackToLocal(AssessmentManager.responses);
        ...
        console.log('🎉 Assessment processed locally', result);
      }
  ...
  setupEventListeners: () => {
    ...
    if (form) {
      form.addEventListener('submit', AssessmentManager.submitAssessment);
      console.log('📌 Form submit listener attached');
    } else if (submitBtn) {
      submitBtn.addEventListener('click', AssessmentManager.submitAssessment);
      console.warn('⚠️ assessmentForm not found, using submit button click listener fallback');
    } else {
      console.error('❌ No assessmentForm or submitBtn found to attach submit listener');
    }
